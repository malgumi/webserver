...
<body>
	<%
		...
		Post Notice_bbs = new PostDAO().getPost(post_id);
		if (!user_id.equals(Notice_bbs.getUser_id())){ //글 작성자 본인이 아닐 경우
			PrintWriter script = response.getWriter();
			script.println("<script>");
			...
			if(request.getParameter("post_title").replaceAll("\\s", "").equals("") || request.getParameter("post_content").replaceAll("\\s", "").equals("")
				...
				PostDAO postDAO = new PostDAO();
					...
					if (result == -1) { //데이터베이스 오류
						...
						script.println("location.href='notice_view.jsp?post_id="+post_id+"'");
						script.println("</script>");
					}
				}
		}
