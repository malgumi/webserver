...
<body>
	<%
		...
		if (!userID.equals(Portfolio_bbs.getPortfolio_userID())){ //글 작성자 본인이 아닐 경우
			...
			if(request.getParameter("Portfolio_bbsTitle").replaceAll("\\s", "").equals("") || request.getParameter("Portfolio_bbsContent").replaceAll("\\s", "").equals("")
					|| request.getParameter("Portfolio_bbsTitle") == null || request.getParameter("Portfolio_bbsContent") == null){ 
				...
				Bbs_PortfolioDAO Portfolio_bbsDAO = new Bbs_PortfolioDAO();
					int result = Portfolio_bbsDAO.update(portfolio_bbsID, request.getParameter("Portfolio_bbsTitle"), request.getParameter("Portfolio_bbsContent"));
					if (result == -1) { //데이터베이스 오류
						...
						script.println("location.href='portfolio_view.jsp?Portfolio_bbsID=" +portfolio_bbsID+ "'");
						script.println("</script>");
					}
				}
		}
